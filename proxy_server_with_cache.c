...
int connectRemoteServer(char* host_addr, int port_num)
{
	...
	// “I want to call IP = 142.250.193.100
	// Port = 80
	// (This is google.com, for example).”
	// The bcopy() line writes the resolved IP address into the structure.
	...
	// free(host_addr);
	return remoteSocket;//Return the connected socket
}


int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
	//storing the request to send to actual server main
	...
	//“I want the remote server to close the connection after the response.
	...
	{
		//“Did the client send a Host: header?”
		if(ParsedHeader_set(request, "Host", request->host) < 0){
			printf("Set \"Host\" header key not working\n");
		}
	}

	//“Let me attach all the headers under the GET line like proper HTTP format.”
	if (ParsedRequest_unparse_headers(request, buf + len, (size_t)MAX_BYTES - len) < 0) {
		printf("unparse failed\n");
		//return -1;				// If this happens Still try to send request without header
	}
		//till here i will ve full http req;

	//“By default, websites use port 80.
	//But if the client explicitly said another port, I’ll use that.”	
	int server_port = 80;				// Default Remote Server Port
	if(request->port != NULL)
		server_port = atoi(request->port);//string to int

	//“Calling the actual website at host:port…” 
	int remoteSocketID = connectRemoteServer(request->host, server_port);

	if(remoteSocketID < 0)//cant get a socket 
		return -1;
	...
	//im getting response i will take nd store
	...
	while(bytes_send > 0)
	{
		bytes_send = send(clientSocket, buf, bytes_send, 0);//send chunks of data to client
		...
		bytes_send = recv(remoteSocketID, buf, MAX_BYTES-1, 0);//receive chunks of data from server
...
void* thread_fn(void* socketNew)
{
    ...
    int* t= (int*)(socketNew);
	int socket=*t;           // Socket is socket descriptor of the connected Client(socket new)
	...
	//to get whole reuest nd headers
	while(bytes_send_client > 0)
	{
		...
		if(strstr(buffer, "\r\n\r\n") == NULL)
		{	
			bytes_send_client = recv(socket, buffer + len, MAX_BYTES - len, 0);//apend data to buffer
		}
		else{
			break;
		}
	...
	//copy to whole buffer to tempreq , req for caching
	...
	if( temp != NULL){
        ...
        // the request
		if (ParsedRequest_parse(request, buffer, len) < 0) //parsing buffer
		{
		...
		{	
			...
			if(!strcmp(request->method,"GET"))	// this project only get requests						
			{
...
int main(int argc, char * argv[]) {
    ...
    //creating the proxy socket--buying new phone
    ...
    // Binding the socket(“OS, please reserve this port and address for me.”)
	...
	while(1)
	{
		...
		if(client_socketId < 0)
		{
			...
			//It saves each client's "phone number" (socket ID) in an array.
...
// Checks for url in the cache if found returns pointer to the respective cache element or else returns NULL
	...
	//sem_wait(&cache_lock);
	//Let me take the cupboard key so no one else interrupts.”
    int temp_lock_val = pthread_mutex_lock(&lock);            //lock 
    ...
    return site;//cache element found return that
}
...
void remove_cache_element(){
    ...
    //sem_wait(&cache_lock);
    int temp_lock_val = pthread_mutex_lock(&lock);   //lock cache
	printf("Remove Cache Lock Acquired %d\n",temp_lock_val); 
	...
	if( head != NULL) { // Cache != empty
		...
		if(temp == head) { 
			...
			p->next = temp->next;	//temp points lru cache element
		}
		...
		free(temp);//deleting the lru element
	} 
	//sem_post(&cache_lock);
    temp_lock_val = pthread_mutex_unlock(&lock);    //unlock cache
	printf("Remove Cache Lock Unlocked %d\n",temp_lock_val); 
}
...
int add_cache_element(char* data,int size,char* url){
    ...
    {   while(cache_size+element_size>MAX_SIZE){
		...
		// place the response data inside
		// write the URL on the folder cover
		// record today’s timestamp
		// put it at the top of the cupboard”
